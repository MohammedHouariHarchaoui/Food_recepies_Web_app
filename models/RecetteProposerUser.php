...
class RecetteProposerUser extends DbModel{
    public $id ='';
    public $nom='';
    public $difficulte='';
    ...
    public function recetteAttributes(){
        return [
            'nom',
            'difficulte',
            'tempPreparation',
    ...
    public function valider(){
        $tableName = $this->tableName();
        $attributes = $this->recetteAttributes();
        echo '<br/>';
        print_r($attributes);
        $params = array_map(function($attribute){return ":".$attribute;},$attributes);
        $statement = self::prepare("
            INSERT INTO recettes (".implode(',',$attributes).") 
        ...
        $statement->execute();
        $this->status = 1;
        $statement = self::prepare("UPDATE recettes_proposer SET status='1' WHERE id=:id");
